...
router.post('/paddle', async (req, res) => {
    ...
    // Ključna izmena: Konvertujemo Buffer u string, kao u GitHub primeru.
    // Ovo osigurava da SDK može pravilno da pročita sadržaj.
    ...
    // Proveravamo da li je event ispravno parsiran pre slanja odgovora
    if (!event || !event.eventType) {
        console.error('❌ Greška pri parsiranju webhooka: eventType je undefined.');
        return res.status(400).send('Webhook parsing failed.');
    }

    // 2. Potpis je validan i događaj je pročitan. Odmah šaljemo odgovor Paddle-u.
    ...
    try {
        ...
        switch (event.eventType) {
            ...
            case EventName.TransactionCompleted:
                ...
                // Sigurnosna provera da li podaci o kupcu postoje
                if (!transaction.customer) {
                    console.warn(`Događaj ${event.eventType} (ID: ${transaction.id}) je stigao bez podataka o kupcu. Preskačem.`);
                    break; 
                }
